...
def test_add_input():
    ...
    recipe.add_input(some_item,1)
    assert len(recipe.inputs) == 1
    assert len(recipe.outputs) == 0
    assert recipe.inputs[0].item == some_item
    assert recipe.inputs[0].quantity == 1
    assert recipe.inputs[0].quantity_per_minute == 10


def test_add_output():
    recipe = Recipe("recipe", None, 6)
    some_item = Item("some  item", "")
    recipe.add_output(some_item, 1)
    assert len(recipe.inputs) == 0
    assert len(recipe.outputs) == 1
    assert recipe.outputs[0].item == some_item
    assert recipe.outputs[0].quantity == 1
    assert recipe.outputs[0].quantity_per_minute == 10
